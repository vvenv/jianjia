...
test('should return original text when translations is empty', () => {
  expect(translate({}, 'Hello world', {})).toBe('Hello world');
});

test('should return original text when translation not found', () => {
  expect(translate(translations, 'Not exists', {})).toBe('Not exists');
});

test('should translate simple text without parameters', () => {
  expect(translate(translations, 'Simple text', {})).toBe('简单文本');
});

test('should translate text with single parameter', () => {
  expect(translate(translations, 'Hello {name}', { name: 'World' })).toBe(
    '你好 World',
  );
});

test('should translate text with multiple parameters', () => {
  expect(
...
test('should handle extra parameters by ignoring them', () => {
  expect(
...
test('should translate text with key and single parameter', () => {
...
test('should translate text with multiple parameters', () => {
  expect(
    translate(translations, '{user} 有 {count} 条消息', {
      user: 'John',
      count: '3',
    }),
  ).toBe('John 有 3 条消息');
